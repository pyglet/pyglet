"""Build and run the isolated WebGL pytest suite in Pyodide and Chromium."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run pytest inside Pyodide against pyglet's WebGL backend.",
    )
    parser.add_argument("--test-dir", default=DEFAULT_TEST_DIR, help="Directory containing WebGL test modules.")
...
def main() -> None:
    ...
    if not test_dir.is_dir():
        raise FileNotFoundError(f"WebGL test directory not found: {test_dir}")
    ...
    with serve_directory(output_dir, args.host, args.port) as server:
        ...
        print(f"Running WebGL pytest suite at: {url}")
        run_webgl_check(
            url,
