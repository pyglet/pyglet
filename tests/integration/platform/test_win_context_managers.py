...
def test_device_context_raises_winerror_when_get_dc_fails(
    ...
    with pytest.raises(patched_win_error_type):
        with context_managers.device_context():
            pass
...
def test_device_context_calls_get_dc(
    ...
    window_handle = 1234
    try:
        with context_managers.device_context(window_handle) as dc:
            pass
    finally:
        patched_get_dc.assert_called_with(window_handle)
...
def test_device_context_calls_release_dc(
    ...
    if release_result == RELEASE_DC_FAILURE:
        with pytest.raises(patched_win_error_type):
            with context_managers.device_context():
                pass
    else:
    ...
    patched_release_dc.assert_called_with(None, dc_handle)
