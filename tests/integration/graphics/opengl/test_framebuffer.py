...
def test_framebuffer_creation_and_binding(window):
    window.switch_to()
...
def test_framebuffer_attach_texture_and_readback(window):
    window.switch_to()

    framebuffer = pyglet.graphics.Framebuffer(context=window.context)
    texture = pyglet.graphics.Texture.create(2, 2, blank_data=True, context=window.context)
    try:
...
def test_framebuffer_attach_depth_renderbuffer(window):
    window.switch_to()

    framebuffer = pyglet.graphics.Framebuffer(context=window.context)
    texture = pyglet.graphics.Texture.create(4, 4, blank_data=True, context=window.context)
...
def test_framebuffer_context_restores_separate_draw_and_read_bindings(window):
    window.switch_to()
    target = pyglet.graphics.Framebuffer(context=window.context)
...
def test_render_texture_context_draws_clears_and_restores_state(window):
    window.switch_to()
    ...
    handler_count = len(window._event_stack)  # noqa: SLF001
    target = pyglet.graphics.RenderTexture(32, 16)
    assert len(window._event_stack) == handler_count  # noqa: SLF001
    rectangle = pyglet.shapes.Rectangle(4, 3, 12, 8, color=(255, 0, 0))

    try:
        with target:
            assert window.camera is target.camera
...
def test_render_texture_restores_custom_camera_auto_viewport(window):
    window.switch_to()
    ...
    try:
        ...
        window.remove_handlers(camera)


def test_render_texture_contexts_can_be_nested(window):
    window.switch_to()
    original_camera = window.camera
    ...
    try:
        with outer:
            assert window.camera is outer.camera
            ...
            with inner:
                assert window.camera is inner.camera
...
def test_texture_render_target_reuses_framebuffer_and_camera(window):
    window.switch_to()
