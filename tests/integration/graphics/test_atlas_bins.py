...
def test_texture_atlas_add_with_border_and_upload(window):
    window.switch_to()
...
def test_texture_atlas_raises_when_no_space(window):
    window.switch_to()
...
def test_texture_bin_creates_new_atlas_when_full(window):
    window.switch_to()
...
def test_texture_array_bin_creates_new_array_when_depth_full(window):
    window.switch_to()
...
@skip_graphics_api(GraphicsAPIGroups.GL2)
def test_texture_array_bin_raises_for_oversized_image(window):
    window.switch_to()
