...
the same as before, but much of the lower level and internal modules have some changes. We hope that this page
...
The secions below should hopefully cover all of the changes that you will need to migrate a project. If you
...
With verion 3.0, to support multiple backends and a more flexible rendering architecture, the graphics backend
...
possible. Minor changes to graphics state and other things however, may no longer need direct OpenGL calls.
...
Instead to draw an image, create a Sprite (or Texture) from it and draw it directly (or batch it). Batched Sprites
are actually a quite efficient way to render a Texture, even if you have a large quantity of them::
...
----------------------------------
:py:meth:`~pyglet.resource.image` previously loaded an image into a texture atlas. However, this was not properly named
as it did not actually return a `~pyglet.image.ImageData` instance in the same way `~pyglet.image.load` does. With the
decisions explained in the previous section, the behavior of this function has been changed.

To go along with that, the :py:meth:`~pyglet.resource.texture` function has also changed slightly. In previous versions
it _only_ returned a standalone Texture instance - there was no automatic texture atlas support. With v3.0 this function
now supports automatically adding to an atlas, mimicing what :py:meth:`~pyglet.resource.image` previously did. You can
still opt to get a standalone Texture if you wish, by passing the ``atlas=False`` argument.
...
libraries that depend on pyglet have long disabled the "shadow window" to work around such issues, so for
v3.0 we are removing it. This will increase compatibility between backends, and lessen the chance of strange
hard to track down driver related bugs.

This change should only affect you if you attempt to load resources before a Window is created. If you want the
previous behavior, it can still be done by simply creating your own window before as shown below::
...
We have added many built in and common states to pyglet to make Groups easier to define and use.
