...
The OpenGL interface is exposed via the :py:mod:`pyglet.gl` module (see :ref:`guide_gl`).

Using the OpenGL interface directly, however, can be difficult to do efficiently.
The :py:mod:`pyglet.graphics` module provides a simpler means for drawing graphics
that uses vertex arrays and vertex buffer objects internally to deliver better performance.

Drawing primitives
------------------

The :py:mod:`pyglet.graphics` module draws the OpenGL primitive objects by a mode
...
:py:func:`pyglet.graphics.draw_indexed` function.  This requires a list of integers
giving the indices into the vertex data.  The following example draws the same
two points as above, but indexes the vertices (sequentially)::
...
:py:func:`pyglet.graphics.draw_indexed` due to pyglet interpreting and formatting
...
this overhead is unnecessarily repeated.

A :py:class:`~pyglet.graphics.vertexdomain.VertexList` is a list of vertices and their attributes, stored in
...
Create a :py:class:`~pyglet.graphics.vertexdomain.VertexList` for a set of attributes and initial data with
:py:func:`pyglet.graphics.vertex_list`.  The following example creates a vertex
list with the two coloured points used in the previous page::
...
constructor.  Otherwise the :py:func:`pyglet.graphics.vertex_list` function takes the same arguments
as :py:class:`pyglet.graphics.draw`, including any number of vertex attributes.
...
the vertex position) appears as an attribute on the :py:class:`~pyglet.graphics.vertexdomain.VertexList` object.  The
...
vertices.  Use :py:func:`pyglet.graphics.vertex_list_indexed` to construct an indexed
...
possible in a single ``draw`` call.  Internally, pyglet uses :py:class:`~pyglet.graphics.vertexdomain.VertexDomain`
and :py:class:`~pyglet.graphics.vertexdomain.IndexedVertexDomain` to keep vertex lists that share the same attribute
formats in adjacent areas of memory.  The entire domain of vertex lists can
then be drawn at once, without calling :py:meth:`~pyglet.graphics.vertexdomain.VertexList.draw` on each individual
list.
...
The :py:class:`~pyglet.graphics.Batch` class implements this functionality, grouping related vertex lists
together and sorting by OpenGL state automatically.  A batch is created with
no arguments::

    batch = pyglet.graphics.Batch()

Vertex lists can now be created with the :py:meth:`~pyglet.graphics.Batch.add` and
:py:meth:`~pyglet.graphics.Batch.add_indexed` methods instead of
:py:func:`pyglet.graphics.vertex_list` and :py:func:`pyglet.graphics.vertex_list_indexed`
functions.  Unlike the module functions, these methods accept a ``mode`` parameter
(the primitive mode) and a ``group`` parameter (described below).
...
you can simply ignore the return value of the :py:meth:`~pyglet.graphics.Batch.add` and
...
With a :py:class:`~pyglet.graphics.Group` these state changes can be encapsulated and associated with the
vertex lists they affect.  Subclass :py:class:`~pyglet.graphics.Group` and override the `Group.set_state`
...
The :py:class:`~pyglet.graphics.Batch` ensures that the appropriate ``set_state`` and ``unset_state``
...
Note the use of an ``__eq__`` method on the group to allow :py:class:`~pyglet.graphics.Batch` to merge
the two ``TextureBindGroup`` identical instances.

Sorting vertex lists
^^^^^^^^^^^^^^^^^^^^

:py:class:`~pyglet.graphics.vertexdomain.VertexDomain` does not attempt to keep vertex lists in any particular order.
...
:py:class:`~pyglet.graphics.Batch` will sort :py:class:`~pyglet.graphics.Group`
objects sharing the same parent by their ``__cmp__`` method.  This allows
groups to be ordered.

The :py:class:`~pyglet.graphics.OrderedGroup` class is a convenience group that does not set any OpenGL
...
group that is drawn before the vertex lists in the "foreground" group::
...
describe an entire scene within a single :py:class:`~pyglet.graphics.Batch`, which then renders it as
