...
class OpenGLES2_Matrices(WindowTransformations):
    def __init__(self, window: Window, backend: OpenGLES2Backend):
...
class OpenGLES2Backend(OpenGLBackend):
    ...
    def get_cached_shader(self, name: str, *sources: tuple[str, ShaderType]) -> ShaderProgram:
        ...
        # With GLES it doesn't support glMatrixMode's and no UBO's, therefore we set the projection for each manually.
        with program:
    ...
    def initialize_matrices(self, window):
        return OpenGLES2_Matrices(window, self)
