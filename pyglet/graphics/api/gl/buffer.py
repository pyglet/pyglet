...
class GLBufferObject(AbstractBuffer):
    ...
    def bind(self) -> None:
        self._context.glBindBuffer(self.target, self.handle)
    ...
    def delete(self) -> None:
        if self.handle is None:
            return
        self._context.glDeleteBuffers(1, GLuint(self.handle))
        self._handle = None
        self._allocated = False
        self._data_uploaded = False
...
class GLIndexedBufferObject(GLBackedBufferObject):
    ...
    def bind_to_index_buffer(self) -> None:
        self._context.glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, self.handle)
...
class GLTransformFeedbackBufferObject(GLBufferObject, TransformFeedbackBuffer):
    ...
    def bind_base(self, index: int) -> None:
        self._ensure_allocated()
        self._context.glBindBufferBase(GL_TRANSFORM_FEEDBACK_BUFFER, index, self.handle)
        self._data_uploaded = True

    def bind_range(self, index: int, offset: int, size: int) -> None:
        self._validate_byte_range(offset, size)
        self._ensure_allocated()
        self._context.glBindBufferRange(GL_TRANSFORM_FEEDBACK_BUFFER, index, self.handle, offset, size)
        self._data_uploaded = True
